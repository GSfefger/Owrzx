										...
										repeat task.wait()
											if game:GetService("Workspace").Enemies:FindFirstChild(Ms) then
												if string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, NameMon) then
													...
													if game:GetService("Workspace").Enemies[Ms].Humanoid.Health == 10 then
														getgenv().SupperFast = false
														UnEquipWeapon(_G.SelectWeapon)
													else
														getgenv().SupperFast = false
...
local window = Library:CreateWindow({ Title = "Owrz Hub x Luxury Hub ", Desc = "ขอบคุณ UI จาก Luxury Hub ลิ้งดิสเจ้าของข้างล่างเลย", serialkey = 111220221700 })
...
windowInstance.Main.MainScreen.JoinDiscordScreen.ButtonJoinDescription.Text = "discord.gg/UtfyWER9F5"
...
local mainTab = window:CreateTab({ Logo = "7566388691", TooltipTitle = "เมนู", TooltipDesc = "Main Script" })
...
spawn(function()
    while task.wait() do
        pcall(function()
            ...
            status_fragment:Refresh("Fragment : ƒ "..game:GetService("Players").LocalPlayer.Data.Fragments.Value)
        end)
    end
end)
                ...
                while wait(0) do
                    if  _G.FastAttackNormalSpeed then
                        if SeraphFrame.activeController then
                            -- if v.Humanoid.Health > 0 then
                                SeraphFrame.activeController.timeToNextAttack = 5
                                SeraphFrame.activeController.focusStart = 6
